...
class LWELattice:
  ...
  def reduce(self, terminateAtGH = True, targetLength = None, maxBlocksize = None, bkzTours = 8 ):
    ...
    if foundSecret:
      self.__vPrint("Found secret while constructing sublattice with i=%d\n" %i)
      self.__vPrint(basis[i-1])
      ...
      self.__vPrint("\n Shortest vector")
      self.__vPrint(basis[0])
    if noKannanEmbedding:
      self.s = self.shortestVector[self.__m:]
    else:
      self.__vPrint("this one!")
  ...
  def __constructBasis(self):
    ...
    for i in range(ctrApproximateHints):
      for j in range(n+1):
          B[j+m+ctrModHints,i+m+ctrModHints] = int(self.__approximateHints[i][j])
              
    #Perfect hints
    for i in range(ctrPerfectHints):
      for j in range(n+1):
          if i + ctrApproximateHints < n:
            B[j+m+ctrModHints,i+m+ctrModHints+ctrApproximateHints] = int(self.__perfectHints[i][j])
          #else:
  ...
  def __constructSubLattice(self,basis):
    ...
    if ctrHints == 0 or self.__modQHintsOnly:
      ...
      #  ].
      top = basis[:m+ctrApproximateHints]
      bottom = basis[m+ctrApproximateHints:]
      
      self.__vPrint("\nTop: %d x %d" %(top.nrows, top.ncols))
      self.__vPrint(top)
      
      self.__vPrint("\nBottom: %d x %d" %(bottom.nrows, bottom.ncols))
      self.__vPrint(bottom)
    
      bottom_left = bottom.submatrix(0,0,bottom.nrows,m+ctrApproximateHints)
      bottom_right = bottom.submatrix(0,m+ctrApproximateHints,bottom.nrows,m+ctrModHints+n+1)
      
      self.__vPrint("\nBottom right: %d x %d" %(bottom_right.nrows, bottom_right.ncols))
      self.__vPrint(bottom_right)
      
      self.__vPrint("\nBottom left: %d x %d" %(bottom_left.nrows, bottom_left.ncols))
      self.__vPrint(bottom_left)
      ...
      dim_bottom = bottom_left.nrows + ctrApproximateHints
    
      B = IntegerMatrix.identity(m+dim_bottom)
    
      for i in range(m+ctrApproximateHints):
        for j in range(m+ctrApproximateHints):
            B[i,j] = top[i,j]
      #self.__vPrint("\nB with top")
      #self.__vPrint(B)
      
      for i in range(bottom_left.nrows):
        for j in range(bottom_left.ncols):
          B[m+ctrApproximateHints+i,j] = bottom_left[i,j]
          #self.__vPrint(B)
        for j in range(bottom_right.ncols-ctrPerfectHints):
          B[m+ctrApproximateHints+i,bottom_left.ncols+j] = bottom_right[i,j+ctrHints]
      
      self.__vPrint("\nB")
      #self.__vPrint(B) 
        #for j in range(dim_bottom):
         # B[m+ctrApproximateHints+i,m+ctrApproximateHints+j] = bottom_right[i,j+ctrHints]
